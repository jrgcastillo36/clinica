@extends('layouts.app')
@section('title', 'Mapa de lesiones · '.$paciente->nombre_completo)

@section('content')
    <div class="page-head">
        <div>
            <h1><i class="fa-solid fa-hand-dots" style="color:#14b8a6"></i> Mapa de lesiones</h1>
        ...
        <div class="flex gap">
            ...
            <a href="{{ route('dermatograma.index') }}" class="btn btn-light"><i class="fa-solid fa-arrow-left"></i> Volver</a>
        </div>
    </div>

    @if(session('ok'))<div class="alert ok mb">{{ session('ok') }}</div>@endif

    <form method="POST" action="{{ route('dermatograma.update',$paciente) }}" id="dermaForm">
        @csrf @method('PUT')
        ...
        <div class="card mb">
            ...
            <p class="muted" style="font-size:12px;margin:6px 2px 0">Elige un tipo y haz clic sobre el cuerpo para marcar la lesión. Puedes describir y eliminar cada una en la lista.</p>
        </div>
        ...
        <div class="grid g-2e">
            ...
            <div>
                <div class="card mb">
                    <h3 class="mb"><i class="fa-solid fa-note-sticky" style="color:var(--violet)"></i> Notas dermatológicas</h3>
                    <textarea name="notas" style="min-height:110px" placeholder="Antecedentes, fototipo, tratamiento…">{{ $derma->notas }}</textarea>
                </div>
                <div class="card">
                    ...
                    @if($derma->exists)
                        <p class="muted" style="font-size:12px;margin-top:10px;text-align:center">Última actualización: {{ $derma->updated_at->locale('es')->isoFormat('D MMM YYYY, HH:mm') }}</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="table-wrap" style="box-shadow:none">
                <table>
                    <thead><tr><th style="width:40px">#</th><th style="width:90px">Vista</th><th style="width:150px">Tipo</th><th>Descripción</th><th style="width:44px"></th></tr></thead>
    ...
    <script>
    (function(){
        const TIPOS=@json($tipos); // {k:[label,color]}
        ...
        function render(){
            ...
            les.forEach((l,i)=>{ const [label,color]=TIPOS[l.tipo]||['',''];
                ...
                tr.innerHTML='<td><b>'+(i+1)+'</b></td>'+
                    ...
                    '<td><input value="'+(l.descripcion||'').replace(/"/g,'&quot;')+'" data-i="'+i+'" class="lesdesc" placeholder="Tamaño, color, evolución…"></td>'+
